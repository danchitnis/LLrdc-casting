...
test.describe('receiver management portal', () => {
  ...
  test('loads the typed Astro dashboard and receives a live snapshot', async ({ page }, testInfo) => {
    ...
    try {
      ...
      const transportSeries = page.locator('[data-latency-series="transport"]');
      const decodeSeries = page.locator('[data-latency-series="decode"]');
      await expect(latencySeries).toHaveCount(4);
      ...
      await transportSeries.click();
      await decodeSeries.click();
      await expect(transportSeries).toHaveAttribute('aria-pressed', 'true');
      ...
      if (receiverState === 'STREAMING') {
        await expect(page.locator('#metrics .metric')).toHaveCount(14);
        await expect(page.locator('#metrics')).toContainText('Estimated total latency');
        await expect(page.locator('#metrics')).toContainText('Encoding latency');
        await expect(page.locator('#metrics')).toContainText('Transport/receiver queue');
        await expect(page.locator('#metrics')).toContainText('Decode/display estimate');
        ...
        await expect(page.locator('#metrics')).toContainText('Avg first packet → playback queue');
