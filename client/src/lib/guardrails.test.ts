...
assert(getCodecString('H264', 30) === 'avc1.42e028', 'H.264 30 FPS must use level 4.0');
assert(getCodecString('H264', 60) === 'avc1.42e02a', 'H.264 60 FPS must use level 4.2');
assert(isFrameRateWithinDisplayMode(60, 60), '60 FPS must be available in an active 60 Hz display mode');
assert(isFrameRateWithinDisplayMode(60, 30) === false, '60 FPS must be disabled in an active 30 Hz display mode');
assert(isFrameRateWithinDisplayMode(60, 0), 'unknown active display mode must not disable 60 FPS');
