...
def run_installer() -> None:
    if os.geteuid() != 0:
        print("[!] Sudo access required for network configuration.")
...
class WpaConf:
    ...
    def create_wpa_conf(self, ssids, user_data: Type[InstallerData]) -> None:
        """Create and save the wpa_supplicant config file with secure permissions"""
        config_dir = get_config_path() + '/cat_installer'
        ...
        wpa_conf = config_dir + '/cat_installer.conf'
        ...
        print("\n=========================================")
        print(" [✓] Eduroam Configuration Generated")
        print("=========================================")
        print(f" CA Certificate: {ca_pem} (chmod 600)")
        print(f" Config File:    {wpa_conf} (chmod 600)")
        print("-----------------------------------------")
        print(" File permissions set to 0600 (Read/Write for owner/root only).")
        print("-----------------------------------------")

        import subprocess
        import shutil
        wpa_sys = "/etc/wpa_supplicant/wpa_supplicant.conf"
        wpa_iface_sys = "/etc/wpa_supplicant/wpa_supplicant-wlan0.conf"

        print("\n[ Writing System Configuration Files ]")
        try:
            shutil.copyfile(wpa_conf, wpa_sys)
            os.chmod(wpa_sys, 0o600)
            shutil.copyfile(wpa_conf, wpa_iface_sys)
            os.chmod(wpa_iface_sys, 0o600)
            print(" [✓] Wrote /etc/wpa_supplicant/wpa_supplicant.conf")
            print(" [✓] Wrote /etc/wpa_supplicant/wpa_supplicant-wlan0.conf")
        except Exception as e:
            print(f" [X] Error writing system configs: {e}")

        print("\n[ Verifying Created Files ]")
        files_to_verify = [ca_pem, wpa_conf, wpa_sys, wpa_iface_sys]
        all_ok = True
        for fpath in files_to_verify:
            if os.path.exists(fpath) and os.path.getsize(fpath) > 0:
                print(f" [✓] Verified: {fpath} ({os.path.getsize(fpath)} bytes)")
            else:
                print(f" [X] Error: File missing or empty: {fpath}")
                all_ok = False

        if all_ok:
            print("\n[ Activating & Verifying Network Connection ]")
            act_script = "/home/danial/tools/activate_eduroam.sh"
            if os.path.exists(act_script):
                subprocess.run([act_script])
            else:
                print(" [!] activate_eduroam.sh not found.")
...
Config.user_realm = "ed.ac.uk"
Config.ssids = ['eduroam']
Config.del_ssids = []
Config.servers = ['DNS:radius01.is.ed.ac.uk']
Config.servers_cn = 'radius01.is.ed.ac.uk'
