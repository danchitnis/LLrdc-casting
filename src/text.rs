...
/// Update only the clock region on an ARGB8888 buffer to prevent screen flickering
pub fn update_clock_argb(buf: &mut [u32], width: u32, height: u32) {
    ...
    let clock_w = time_str.len() * 8 * scale;
    let clock_h = 8 * scale;
    let clock_x = box_x + box_w - clock_w - 20 * scale / 2;
    let clock_y = box_y + (header_h - clock_h) / 2;

    let w = width as usize;
    let h = height as usize;

    // Clear only the clock rectangle with header_bg
    ...
    // Draw updated clock string over cleared background
    draw_string_argb(
        buf,
        width,
        height,
