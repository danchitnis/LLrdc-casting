...
pub fn spawn_registration(state: PairingState, cert_hash: String) {
    tokio::spawn(async move {
        let worker_url = match std::env::var("PAIRING_WORKER_URL") {
            ...
            _ => {
                state.set_status("Cloud discovery not configured");
                return;
            }
        };
        let receiver_id = match std::env::var("RECEIVER_ID") {
            ...
            _ => {
                state.set_status("Cloud discovery identity unavailable");
                return;
            }
        };
        let key = match std::env::var("RECEIVER_REGISTRATION_SECRET").ok().and_then(|value| URL_SAFE_NO_PAD.decode(value).ok()) {
            ...
            _ => {
                state.set_status("Cloud discovery credential unavailable");
                return;
            }
        };
        ...
        loop {
            let Some(ip) = crate::net::get_preferred_private_ipv4() else {
                state.set_status("Cloud discovery waiting for network");
            ...
            match result {
                Ok(response) if response.status().is_success() => {
                    ...
                    state.set_status("Ready - direct LAN pairing available");
                ...
                Ok(response) => {
                    ...
                    state.set_status("Cloud discovery unavailable - direct LAN pairing available");
